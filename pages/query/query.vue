	...
	<view>
		<view v-if="userToken">
			<view class="list" v-for="(item, index) in lists" :key="index">
				<view @click="country(item.id,item.type)">
					<text>{{ item.countryName1 }}</text>
					<image src="../../static/image/zjt.png"></image>
					<text>{{ item.countryName2 }}</text>
				</view>
				<view @click="country(item.id,item.type)" v-if="item.type == 1">查询类型：已知多边HScode</view> 
				<view @click="country(item.id,item.type)" v-else-if="item.type == 2">查询类型：仅知商品描述</view> 
				<view @click="country(item.id,item.type)" v-else>查询类型：已知单边HScode</view> 
				<view @click="country(item.id,item.type)">{{ item.createTime | formatDate }}</view>
				
				<view @click="shouca" v-if="item.isCollect == '1'" class="shoucang">{{i18n.shouc}}</view>
				<view class="shoucang azsxas" @click="shouchang(item.id)" v-else>{{i18n.weishouc}}</view>
			</view>
...
export default {
	...
	methods: {
		...
		getmorenews: function() {
			...
			uni.request({
				url: this.baseUrl + '/api/log?is_collect=0&count=10&page=' + page,
				method: 'GET',
				header: {
